...
Last updated: 2026-09-21 after Safety and mobile-navigation publication and session closeout, with public behavior verified through PR #53 / `9443316`. The filename is retained for existing links; earlier release records retain their dated status.
...
The original checkout remains on `codex/september-18-handoff-audit` with its pre-existing uncommitted edits to `README.md`, `_staff/README.md`, this handoff, and `_staff/site-todo.md`. Those edits belong to a separate September 18 audit and were preserved without incorporation into this session's releases. Start new work from current `origin/main` in an isolated worktree; do not treat that older checkout as the latest release record or discard its edits.
