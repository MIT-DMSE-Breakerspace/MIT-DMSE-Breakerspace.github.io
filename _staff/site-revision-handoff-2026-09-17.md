...
Last updated: 2026-09-21 after mobile Calendar publication and session closeout, with public behavior verified through PR #56 / `66e4de1`. The September 18 audit and Safety/mobile-navigation release records remain below. The filename is retained for existing links; earlier release records retain their dated status.
...
For page-by-page editorial work, the owner wants analysis and suggestions, then a locally hosted draft for manual review, followed by publication only after approval. Shorten copy without losing meaning or operational facts, and prioritize useful actions and readable organization on phones. Approval of the completed Safety and navigation releases does not approve the next page's draft.
...
## Mobile Calendar Revision — September 21

Published in [PR #56](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/56), implementation `99a1105`, merge `66e4de1`. The owner reviewed the local preview and authorized publication and session closeout on September 21. Calendar now uses LibCal's Upcoming Events widget at widths up to 720px. Configuration: calendar `19408`, upcoming mode (`m=upc`), ten events (`l=10`), titles with dates and start/end times (`simple=ul_date`), and no category/audience/campus/tag filters. Training sessions, assistant shifts, and lab/lounge reservations remain together. The list is an upcoming-events preview; the full-calendar links above and below it provide other dates. The existing desktop iframe URL and 600px height are unchanged.

The native Week List (`dv=listWeek`) was inspected at 320px, but its separate time column still squeezed long event titles. The selected widget follows Training's existing approach: fetch the public CORS-enabled endpoint, allow only LibCal event links and date text, and render with site styles. `assets/js/calendar.js` selects the initial view with `matchMedia`, fetches the list only on mobile, and instantiates the desktop iframe from an inert template only on wider screens. Resizing switches the visible view and reuses already loaded content. The full-calendar and email links remain available when JavaScript or LibCal is unavailable; empty and failed feeds have explicit messages.

Validation: Jekyll build (existing Sass warnings), all-site HTML/internal links and content checks, JavaScript syntax, `git diff --check`, and Calendar Pa11y at 390×844 and 1280×900 passed. Live events wrapped without horizontal overflow at 320×740 and 390×844; all ten events and start/end times appeared, with no desktop iframe on an initial phone visit. Desktop initial loading and mobile/desktop resizing selected the intended view without duplicate widgets. Empty and HTTP-error responses were checked using temporary local fixtures. The original static desktop iframe and the revised desktop iframe both appeared blank in the local browser, while the production embed loaded; desktop iframe contents could not be verified locally. No registration, booking, or LibCal administration was performed.

Release verification passed: the PR's checks, merged-main [Site quality run 35641606003](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/actions/runs/35641606003), and [Pages deployment 35641605288](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/actions/runs/35641605288) all succeeded. The live stylesheet identified merge `66e4de1`. At 390×844 and 320×740, the live list displayed all ten events with dates/start/end times, no desktop iframe on a fresh phone visit, and no horizontal page overflow. At 1440×1000, the desktop iframe rendered its events and controls correctly with no mobile-feed load. Resizing from phone to desktop also loaded the calendar; Next and Previous switched weeks and returned to the current week. This resolves the local iframe verification limitation above. Keyboard activation of a mobile event had already opened the correct LibCal registration page during local review; no registration was submitted.

Session cleanup replaced the local browser preview with the published Calendar page and stopped this session's preview server on `127.0.0.1:4173`. The mobile-calendar queue item is complete. The instrument-renovation pause and other open policy/maintenance tasks remain in effect.
...
The owner reviewed the mobile logo/Menu and section-heading visibility preview and authorized publication on September 21. [PR #52](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/52) and the compatibility fix in [PR #53](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/53) are merged; the final public behavior release is `9443316`. Its [main CI](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/actions/runs/35637171278) and [Pages deployment](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/actions/runs/35637170986) passed. Live verification confirmed that revision in the stylesheet URL, all 12 menu links, opening/Escape/focus behavior, visible Safety and FTIR section headings, no phone overflow, and the desktop footer in its sidebar.

The initial merge `1f161ae` passed local/CI Dart Sass but failed before deployment because Pages' Ruby Sass 3.7.4 rejected the menu shadow's space-separated `rgb()` syntax. PR #53 uses equivalent comma-separated `rgba()` syntax. The failure was reproduced with Ruby Sass 3.7.4, and the full corrected stylesheet passed both that compiler and the normal Jekyll build. Aligning future compiler checks with production remains a separate checklist item; no deployment failure remains open for this release.
...
## Session Closeout — September 21

Safety and the shared navigation update are committed, merged, deployed, and verified live. No draft from this session remains pending approval. Temporary Jekyll previews on ports 4000 and 4001 were stopped; use the [live Safety page](https://breakerspace.mit.edu/safety.html) for further feedback. The clean source worktrees and published branches are retained.

The owner subsequently authorized integrating the previously uncommitted September 18 audit. Its original changes were preserved in commit `17e82fd` on `codex/september-18-handoff-audit`, then reconciled with the September 21 releases. The four planning documents now include the complete September 18 release table, corrected page summaries and deployment records, current 24/7/365 access, Safety and mobile-navigation releases, and the owner-review workflow. This supersedes the earlier note that the audit remained separate; no audit content is being discarded or left uncommitted. Start new work from current `origin/main` and inspect Git status before editing.

The remaining related work is the compiler-check maintenance item, optional native-device/screen-reader verification, and the already-open supervised-course-participation policy question. The broader page-review queue and instrument-renovation pause remain in force.
...
| Training/calendar | `training.md`, `calendar.md`, `_includes/training-sessions.html`, `assets/js/training-sessions.js`, `_includes/breakerspace-calendar.html`, `assets/js/calendar.js` |
...
At the PR #42 checkpoint, the live homepage referenced merge `985dded`'s stylesheet, which no longer contained the 72ch cap and retained the 1320px frame. The end-of-day audit verified successful Site quality and Pages runs for every September 18 merge. The last September 18 release was `9d29e33`: [Site quality run 35381552957](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/actions/runs/35381552957) and [Pages deployment 35381551818](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/actions/runs/35381551818). Deployment success does not complete the owner's broader live-page review.

The original September 18 documentation audit recorded a fresh Jekyll build (existing Sass deprecation warnings), `git diff --check`, coverage of all 25 commits in the release table, and checks of all 46 local file/fragment links across its four documents. Those counts describe that audit's version; the September 21 integration adds the later records. The graphical findings above remain the recorded release checks; this audit reviewed commits, documentation, and deployment records.

The September 21 integration passed a fresh Jekyll build, whitespace checks, and all 51 local document file/fragment links. It reverified the 25-commit table, all 11 merge trees, and successful CI/Pages runs for those merges. The current access, Safety, and mobile-navigation sections were preserved exactly; only the four planning documents differ from the previously published branch.

September 18 end-of-day cleanup stopped that day's remaining Jekyll preview at `127.0.0.1:4000`. The separate September 21 preview cleanup is recorded above.

Follow the root README's branch-and-reviewed-PR workflow for further changes, keeping releases small and preserving anchors. The remote uses the `github-mit` SSH alias; specify `--repo MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io` with GitHub CLI commands. The latest public behavior change recorded here is `66e4de1`; later documentation-only commits may follow it. Inspect current Git status before starting; do not assume an older branch or preview process still exists.
...
> Continue the Breakerspace website usability work from current `origin/main` in an isolated worktree. Read `_staff/site-revision-handoff-2026-09-17.md` (updated September 21, public behavior through PR #53 / `9443316`) and the September usability section of `_staff/site-todo.md`; consult the original review only for a specific unresolved question. The September 18 release audit is integrated, and Safety and the sticky mobile logo/Menu update are live. Keep Training focused on registration and Reserve Time focused on booking. Start with my live-page feedback; Teach With Us is the next non-instrument copy candidate. Keep each editorial task focused on one page, preserve meaning and operational facts, and make it useful on phones. Present analysis, then a local draft for my manual review, and publish only after approval. Further instrument-page renovations remain paused. Use subagents for bounded graphical checks and return concise findings without page renderings in the main task. Organized user trials are not required.
