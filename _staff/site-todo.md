...
* [ ] Complete the three remaining Moira list renames so every group name is `dmse-brkrspc-` plus the instrument's page slug: `-dsx` → `-optical`, `-duetta` → `-uv-vis`, and `-utm` → `-instron`. All three are access-controlling lists, so change each list name and its WIN workstation logon privileges in the same operation and confirm a real login afterward. The two comms-only renames (`-hrdnss` → `-hardness-tester`, `-ionmill` → `-ion-mill`) were completed 2026-07-31 and confirmed that WebMoira renames cleanly and accepts a 28-character name. When all five are done, mark the rows and delete the rename-status section in [access-and-logistics.md](../_staff-training/access-and-logistics.md).
