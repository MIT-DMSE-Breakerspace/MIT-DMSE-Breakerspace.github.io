...
* [ ] During the second wave, retrofit the optical pilot to the continuous session-checklist model and practice the training with participants using the printed Quick Guide as the routine-workflow aid. Cover the complete normal-use path from the expected ready state; leave conditional recovery and unusual-startup details on the canonical page when the guide directs users there.
* [ ] **Print-proof the FTIR Quick Guide on Letter duplex.** The candidate now renders as two explicit pages without clipping or overlap. Test 100% print-scale legibility, grayscale reproduction, QR scanning, and long-edge flip; revise from the physical proof rather than a stale word-count budget.
