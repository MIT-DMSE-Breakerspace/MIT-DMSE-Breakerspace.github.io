...
Last updated: 2026-09-21 for the 24/7/365 access decision, Safety publication, and approved mobile navigation revision; other work retains its dated review records below.
...
* [x] Review and approve the separate sticky mobile navigation revision on `codex/mobile-sticky-navigation`: keep the compact logo/Menu row visible, scroll the expanded links within the viewport, and keep section headings and keyboard focus below the bar. The owner approved the preview and authorized publication on September 21. Checks and release tracking are recorded in the handoff's Mobile Navigation Revision entry. The existing desktop sidebar and destinations remain unchanged.
* [ ] Add a stylesheet compatibility check matching the production Pages compiler, or align the build environments in a separate maintenance change. PR #52 passed local/CI Dart Sass but Pages' Ruby Sass 3.7.4 rejected modern `rgb()` syntax; the immediate release fix uses equivalent `rgba()` syntax. Keep Pages deployment verification part of release completion.
