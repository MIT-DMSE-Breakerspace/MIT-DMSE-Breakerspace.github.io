...
* [ ] Publish and verify the owner-reviewed September 21 mobile Calendar revision. At widths up to 720px, it loads LibCal's Upcoming Events widget with the next ten events, all categories, and dates/start/end times; wider screens retain the existing week calendar. The full-calendar link now precedes either view. Local phone layout, conditional loading, resize behavior, empty/failed responses, build, links, content, and automated accessibility checks passed. The unchanged desktop iframe also appeared blank in a static local baseline, so the local browser could not verify its contents; the production embed was readable. See the handoff's Mobile Calendar Revision record. Training retains its separate compact session list. The owner authorized publication on September 21.
