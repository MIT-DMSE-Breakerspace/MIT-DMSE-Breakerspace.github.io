...
* [ ] Add a complete SOP coverage map to the optical pilot staff guide, then practice both trainings with participants using the printed Quick Guide as the routine-workflow aid. FTIR's map was added 2026-08-03.
* [ ] **Print-proof the FTIR Quick Guide on Letter duplex.** Side 2 is roughly 582 words after four correctness rounds and has never been printed. If the two-page budget does not hold, the fix affects every guide written after it, so test before retrofitting optical and XRD. Options if it fails: fold the Save details into Collect, compress step 8 into a pointer, or use the overflow rule and add a task-specific card.
* [ ] Audit, print-test, laminate, and install the optical and XRD Quick Guides under the [content contract](../instruments/readme.md#quick-guide-content-contract). FTIR is retrofitted and is the model to copy.
