...
* [x] Complete the Instron operating deep-dive: Bluehill method setup, load-cell and fixture changes, travel limits, each offered test type, video extensometer, saving and export. Written 2026-07-29 with page-level citations to the now in-repo 6800 operator guide. Two follow-ups remain in [instruments/staff-todo.md](../instruments/staff-todo.md): the **reviewed example force-displacement curve** (the one outstanding content item, described in prose meanwhile) and staff confirmation of the workstation's actual method names, export conventions, and recommended end-of-test condition.
