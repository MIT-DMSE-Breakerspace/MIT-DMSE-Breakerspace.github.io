...
* [ ] Resolve the audit's cross-cutting layout finding before any Quick Guide print proof: the trained-user notice overlaps the SOP heading in the paginated print path on all eleven prototypes, including the FTIR reference. It is locked header geometry and needs a staff decision, and it must not be resolved by shrinking type, dropping content, or trimming a canonical instrument identity.
* [ ] **Standardize the Quick Guide source location before promoting any prototype.** The published guides currently use thin `quick-guides/<slug>.md` wrappers whose full HTML content lives in `_includes/quick-guides/<slug>.html`, while the provisional SOP/visual-map files embed their full HTML directly in top-level `quick-guides/*-sop-map-prototype.md` documents. Choose and enforce one structure for the final guide set, defaulting to the existing include-backed publishing contract unless that architecture is explicitly changed; migrate the prototypes and published guides consistently, update the template and production procedure, and add a check that prevents mixed source-location conventions from returning.
