...
Last updated: 2026-09-22 with owner approval of the compact mobile mini-calendar; publication verification is pending. Other work retains its dated review records below.
...
* [x] Publish the owner-reviewed sticky mobile navigation ([PR #52](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/52) and [compatibility fix #53](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/53), final release `9443316`): keep the compact logo/Menu row visible, scroll expanded links within the viewport, and keep section headings and keyboard focus below the bar. PR/main CI, Pages deployment, and live checks passed. Desktop sidebar and destinations remain unchanged. See the handoff's Mobile Navigation Revision entry for coverage and limits.
* [ ] Add a stylesheet compatibility check matching the production Pages compiler, or align the build environments in a separate maintenance change. PR #52 passed local/CI Dart Sass but Pages' Ruby Sass 3.7.4 rejected modern `rgb()` syntax; PR #53 resolved deployment using equivalent `rgba()` syntax. Keep Pages deployment verification part of release completion.
* [x] Complete the initial Get Trained simplification ([PR #34](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/34)). Its preparation/access sections were subsequently removed by the owner's registration-focused revision in PR #48 below.
* [x] Publish the registration-focused Get Trained revision (September 18, [PR #48](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/48), `6d64e6d`): retain the short process overview and one-time online-record action, and replace the full calendar embed with five live upcoming Instrument Training sessions plus static schedule/Calendar links. Remove preparation, access-setup, and project/course sections; access setup is explained during training. Preserve registration fragments and the empty/failed/no-JavaScript fallbacks. See the handoff for checks and limits.
...
* [x] Publish and verify the owner-reviewed September 21 mobile Calendar revision ([PR #56](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/56), merge `66e4de1`). At widths up to 720px, LibCal's Upcoming Events widget shows ten events from all categories with dates/start/end times; wider screens retain the existing week calendar. The full-calendar link precedes either view, and empty/failed-feed and no-JavaScript fallbacks remain. PR/main CI and Pages deployment passed. Live checks at 320×740, 390×844, and 1440×1000 verified the list, conditional loading, no horizontal page overflow, and working desktop calendar/navigation; the localhost iframe limitation was resolved by live verification. See the handoff's Mobile Calendar Revision record. Training retains its separate session list.
* [ ] Publish and verify the owner-approved September 22 mobile mini-calendar follow-up. The widget adds month navigation and date selection, starts with five upcoming events across all categories, and hides past events. The owner opened the LAN preview successfully on a phone and confirmed date selection; reported excess whitespace led to a compact 550px iframe with `h=450`. The owner approved this final sizing and authorized publication. The iframe cannot automatically follow its cross-origin contents, so short event lists still leave some space. See the handoff's Mobile Mini-calendar Revision record for configuration and checks.
...
* [x] Publish the Data Tools cleanup (September 18, [PR #47](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/47), `e998d5f`): shorten the introduction and remove the outdated library-loading note, generic usage steps, and interpretation disclaimer. Keep the device recommendation, launch destinations, and separate Carbonio maintenance boundary. Fit FTIR/Raman/XRD cards three across when space permits, wrap on narrower screens, and align launch links without shrinking text. See the handoff's Data Tools record for release checks.
* [x] Publish Get Help's focused editorial pass (September 18, [PR #49](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/49), `9d29e33`): put contact options first, simplify access/files guidance to user actions, remove internal administration, and use wrapping folder sequences. The owner removed Quick Links because the shortened page is easy to scan. Existing anchors, operational safeguards, and shared-folder privacy guidance are preserved. See the handoff's Help & Support record for checks and limits.
* [x] Publish the owner-reviewed Safety and Lab Use editorial revision (September 21, [PR #51](https://github.com/MIT-DMSE-Breakerspace/MIT-DMSE-Breakerspace.github.io/pull/51), merge `0d47672`). CI and Pages deployment passed; the live page was verified. A compact emergency notice and early contents list lead to Emergencies and incidents, Working safely, and Lab-use rules. Before You Work, Related Pages, and repeated booking/data reminders are removed; sample restrictions have one home; food and samples precede PPE; injury reporting stays with incidents. Safety requirements and original fragments remain. See the handoff's Safety editorial record for counts, checks, and limitations. The supervised-course-participation policy question remains open; the guest rule is unchanged.
