...
[Experiment setup](../assets/img/tutorials/ftir/experiment-setup.JPG) allows you to change the parameters to fit your requirements. For faster collections, reduce the number of scans and/or scan resolution, and note how the estimated time changes. For samples with short peaks that can be hidden in noise, increase the number of scans to improve the signal to noise ratio.
