...
**The compact FTIR trainer guide is ready for release after lab-manager review on 2026-08-11.** Background-preview acceptance, System Status recognition, the complete background-recovery cycle, and the participant artifact were resolved in context. The available printed FTIR Quick Guide supports the session; review and physical proof of the separate compact Quick Guide redesign do not block this trainer guide.

Atmospheric and contamination band positions remain a possible canonical-page enhancement, and the verified background-recovery sequence still needs to be copied into that page when a published-page edit is authorized. Neither is a trainer-guide release blocker. A future practice session may still improve pacing and wording after release.
...
The lab manager authorized a cross-instrument **provisional staff-guide composition pass**, and the pass was completed on 2026-08-10. It produced `published: false` trainer-run-sheet prototypes for FTIR, Phenom XL, Phenom Pure, XRD, and Instron while preserving every operational blocker as a specific review gap. FTIR completed review and became ready for release on 2026-08-11; the other four remain visibly incomplete. The procedure remains in [`provisional-draft-procedure.md`](provisional-draft-procedure.md), results are in [`provisional-draft-work-orders.md`](provisional-draft-work-orders.md), and the original copy-paste assignment is archived in [`CODEX-CLI-PROMPT.md`](CODEX-CLI-PROMPT.md).
...
* `ftir-trainer-prototype.md` is the reviewed, unpublished FTIR release candidate. The Phenom XL, Phenom Pure, XRD, and Instron prototype files remain incomplete first-wave review artifacts. None replaces a live staff guide until its release step is authorized.
...
The same authorization governed the completed provisional **staff-guide** pass under [`provisional-draft-procedure.md`](provisional-draft-procedure.md). FTIR completed lab-manager review on 2026-08-11 and is ready for release while remaining unpublished. The other four prototypes remain trainer-run-sheet and evidence inventories with unresolved choices. All remain separate from live guide filenames and use prototype-only shared includes until an explicit release step.
