...
Last substantive work: 2026-09-16.
...
The lab manager authorized a cross-instrument **provisional staff-guide composition pass**, and the pass was completed on 2026-08-10. It initially produced trainer-run-sheet prototypes for FTIR, Phenom XL, Phenom Pure, XRD, and Instron while preserving every operational blocker as a specific review gap. FTIR completed review on 2026-08-11 and was released on 2026-08-31. On 2026-09-14 the two Phenom drafts were replaced by one availability-flexible SEM guide, with their old URLs retained as redirects. The combined SEM, XRD, and Instron guides remain published with a **Draft advisory** state and are unapproved for training delivery. The procedure remains in [`provisional-draft-procedure.md`](provisional-draft-procedure.md), results are in [`provisional-draft-work-orders.md`](provisional-draft-work-orders.md), and the original copy-paste assignment is archived in [`CODEX-CLI-PROMPT.md`](CODEX-CLI-PROMPT.md).
...
### Sample-Flexible Guides {#sample-agnostic-guides}

**Decided 2026-08-05 and clarified 2026-09-14. Neither the Quick Guide nor the staff guide names a required default characterization sample, sample-library identifier, or cabinet location.** A participant may bring a candidate sample, or the lab may supply one. The trainer screens any participant sample against the documented limits and keeps a known-good lab sample available so safety, pacing, and image-quality practice do not depend on an unfamiliar sample. A fixed calibration or system-check reference may be named only when the canonical workflow genuinely requires it, as with the Raman silicon standard.

The reasoning:

* **The sample should fit the audience.** A subject exercise, a research question, and a general session may use different samples through the same workflow. A required named default works against that flexibility.
...
* **The workflow is the durable content.** Who supplies the sample is a per-session choice; safe compatibility, operation, and image-quality decisions belong in the maintained document.
...
* **A known-good fallback** — when a participant sample behaves unexpectedly or is unsuitable, the session moves to the prepared lab sample instead of asking the trainer to diagnose an unfamiliar material from memory.
...
This is a scoping decision for the current round, not a rejection of approved samples. A later round may name optional sample-library examples without making one material a prerequisite for training.
...
The eleven-file provisional set was audited on 2026-08-10; the findings are in [`../_staff/quick-guides/audit-2026-08-10.md`](../_staff/quick-guides/audit-2026-08-10.md). Two results bear directly on this layout model:

* **The compact composition does not survive a long instrument name.** Six instruments wrap the document title to two lines, which pushes the trained-user notice into the SOP heading and obscures one to two lines in print. FTIR, with a short title, never exercised this case, so the model was validated against its easiest input.
* **A two-page render is not evidence the layout holds.** All eleven files render as exactly two Letter pages while overlapping. This is a third instance of the general lesson below: the defect is invisible in the HTML, the build, the page count, and a scrolling screenshot.
...
**Adopted 2026-08-05 for FTIR and corrected after lab-assistant review on 2026-08-10.** One continuous run sheet remains the intended delivery shape; the current compact `ftir.md` illustrates it with selective participant observations. The combined Phenom draft added a useful variant on 2026-09-14: one chronological shared workflow can contain short, unmistakable hardware branches when instrument availability varies, provided skipped branches do not become incomplete checklist items. Instron remains the structural exception because its workflow genuinely crosses the physical load string, Bluehill, and the handset. Exact measurements and validation results remain in the [rolling work-order ledger](provisional-draft-work-orders.md#status), not duplicated here.
...
* **Orient trainers to the durable references before instrument operation.** In the scope/startup phase, the second instrument-specific checkbox shows the SOP, detailed instructions, and common failure modes on the canonical instrument page and states that instrument's stop conditions. Keep this in the same position across guides, immediately after the checkbox defining what the training covers.
...
| --- | --- | --- | --- |
| FTIR | Two trainer-selected solids through the routine ATR cycle. The background preview is not an acceptance test; contamination is recognized from negative or unexpected features in the collected sample spectrum. A green **System Status** check means ready; any other state stops collection. | The available printed FTIR Quick Guide supports the session; its compact redesign remains a separate project. | Trainer guide released 2026-08-31; optional canonical-page enhancements remain separate. |
| Phenom SEM | One combined guide may be delivered on Pure, XL, or both; completion does not require both microscopes. Shared scope is sample screening, NavCam, SEM imaging with BSD/SED, focus, automatic brightness/contrast for display, accelerating voltage, beam intensity, vacuum mode, **Scan Size 960 × 540**, **Medium Averaging**, acquisition, data handling, and shutdown. Pure uses its room-temperature holder and below-holder-edge rule; introduce but do not demonstrate the temperature-controlled stage. XL Level 1 uses high vacuum and its removable-tray/dial rule; preview EDS when XL is available, or explain it from the operating page when unavailable. | Keep separate Pure and XL Quick Guides and interface sources. The combined trainer guide does not authorize merging the physical loading rules or primary Quick Guides. | Combined draft revised 2026-09-16; review the revised wording and run a practice session. Quick Guide review and physical proof remain separate. |
...
| Optical | Best Image; quick 2D stitch as map image; map navigation; switch from the 3x to 10x objective; normal 2D capture; slower high-quality 2D area stitch with **Use as map image** deselected; and 3D capture. Introduce high-quality 3D stitching and LEXT analysis/reporting, with selected analysis training as an optional extension when time and availability allow. | Map the taught DSX functions. Keep detailed LEXT analysis controls off the primary map. Review the triaged multi-state candidates and matched focus pair, then lock and integrate an optical-specific layout. | The existing training workflow was codified, structurally retrofitted, and operationally reviewed on 2026-09-11. The trainer guide is current; Quick Guide layout, capture integration, QR, and physical proof remain separate work. |
...
| Hardness tester | One single-point measurement using the programmed `Vickers 300 gf` test, reported as `HV 0.3`; leave load, dwell, and other test settings unchanged. | Current proposed map and indent-quality pair remain appropriate; use the captured HV 0.3 state. | Reconcile installed hardware/scales; integrate current captures; approve quality behavior; confirm save/export and end state. |
...
| Second | **Hardness tester** | Provisional composition draft; needs capture integration and operational check | Missing | Level 1 is the programmed Vickers 300 gf / HV 0.3 test with settings unchanged; reconcile fitted hardware/scales and approve the captured controls, quality behavior, save/export, and final state. |
...
The provisional **staff-guide** composition pass was completed on 2026-08-10. FTIR completed review and was released on 2026-08-31. The two Phenom artifacts were consolidated on 2026-09-14; the combined SEM, XRD, and Instron artifacts are published as incomplete **Draft advisory** review aids. Review the combined SEM rewrite first, then XRD for its split touchscreen/workstation/log-book path, and Instron separately as the deliberate multi-surface structural exception. This sequence organizes document review; it does not change the co-equal first-wave rollout priority.

**SEM trainer-guide rewrite — 2026-09-14:** The separate Phenom XL and Pure drafts were replaced by one guide that can be delivered on either working instrument. It shares sample screening, NavCam, imaging, data, shutdown, and closeout; it branches explicitly for loading, unloading, and instrument-specific capabilities. A participant-provided screened sample or lab-provided sample may be used, with a known-good lab fallback. The rewrite preserves the 2026-09-11 technical decisions and the canonical Pure and XL unloading actions but resets editorial review to the beginning.
...
* **Neither artifact requires a default characterization sample, sample-library identifier, or cabinet location.** A participant-provided screened sample or lab-provided sample may be used; keep a known-good lab fallback. A canonically required calibration or system-check reference may still be named. Decided 2026-08-05 and clarified 2026-09-14; see [sample-flexible guides](#sample-agnostic-guides).
...
* Hardness-tester Level 1 uses the programmed `Vickers 300 gf` test, reported as `HV 0.3`. Trainers and users leave its load, dwell, and other settings as programmed; choosing another approved test is a sample-compatibility decision, not a settings-editing exercise. Decided 2026-08-12.
...
* The criteria a participant- or lab-provided characterization sample must satisfy, what disqualifies it, and the known-good fallback. Do not require a default material or cabinet location; canonically required calibration/system-check references are the narrow exception. See [sample-flexible guides](#sample-agnostic-guides).
...
Develop the Quick Guide and staff guide together once the corresponding public instrument workflow and Level 1 exercise are stable enough to train consistently. Before routine delivery, each pair needs stated sample-selection criteria, a recognizable image-quality decision, and a reviewed laminated Quick Guide at the instrument. **A specific approved sample is not a prerequisite** — see [sample-flexible guides](#sample-agnostic-guides); use a screened participant sample or a lab sample and keep a known-good fallback.

The lab manager authorized one exception on 2026-08-10: a cross-instrument **provisional composition pass** may add visibly incomplete Quick Guide drafts using the standardized descriptive frames and review-gap blocks in [`../_staff/quick-guides/README.md`](../_staff/quick-guides/README.md#provisional-placeholder-draft-mode). On 2026-09-11 the compositions were authorized for web publication through a warning-labeled staff-review index. That review publication does not settle curriculum, remove technical gates, approve printing or installation, or make a staff guide complete. Technical completion, paired staff-guide review, print proof, and installation still proceed one instrument at a time. Track both the provisional pass and eventual rollout in [`../_staff/site-todo.md`](../_staff/site-todo.md).

The same authorization governed the completed provisional **staff-guide** pass under [`provisional-draft-procedure.md`](provisional-draft-procedure.md). FTIR completed lab-manager review on 2026-08-11 and was released on 2026-08-31. The combined SEM, XRD, and Instron prototypes remain trainer-run-sheet and evidence inventories with unresolved choices; they stay separate from live guide filenames and use prototype-only shared includes until an explicit release step.
