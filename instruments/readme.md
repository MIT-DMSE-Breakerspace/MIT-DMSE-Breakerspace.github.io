...
Quick Guides currently exist for `/quick-guides/optical/`, `/quick-guides/ftir/`, and `/quick-guides/xrd/`, backed by the matching includes in `_includes/quick-guides/`. Use their page structure and the shared `quick-guide-*` CSS classes as the pattern for later instruments. The optical and FTIR guides are the original pilots and predate the scope rules above, so they still contain some interpretation guidance; `xrd.html` follows the current rules and is the better structural model. Its Side 1 also runs longer than the pilots and has not had a print check, so verify the two-page budget before copying its density.

**Write the staff training guide first, then the Quick Guide.** The training guide is where the decision gets made about which of an instrument's many capabilities a beginner training actually covers. That scope then becomes the Quick Guide's scope: an illustrated, annotated walkthrough of the same operations, plus the core startup and shutdown reminders. Built in that order the Quick Guide reinforces what a trainer taught, and the two cannot disagree about what a routine session involves. XRD's Quick Guide was written before its training guide, so treat its scope as provisional. See [`../_staff-training/README.md`](../_staff-training/README.md).
