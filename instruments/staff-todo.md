...
* [ ] Build and review the authorized 2026-08-10 cross-instrument provisional composition set. Each draft must remain unpublished and visibly incomplete, reproduce the full canonical SOP on Side 1, use only standardized descriptive frames for unavailable images, and preserve unresolved curriculum or technical choices as review gaps. This pass inventories composition and evidence; it does not make any blocked guide technically complete or usable at an instrument.
...
* [x] **Resolve background-preview acceptance.** Staff confirmed 2026-08-11 that the complex background shape is not a practical acceptance test because contamination usually changes it subtly. For Level 1, confirm the crystal is clean, dry, bare, and unclamped; recognize background problems later from negative or unexpected features in the collected sample spectrum.
...
* [ ] **Add the confirmed background-redo cycle to the SOP.** Staff confirmed 2026-08-11: gloves on → release the pressure tower and unload the sample → clean and dry the crystal and plate → gloves off → collect a complete new background on the bare, unclamped crystal → reload and recollect the sample. If negative features remain, stop and ask staff. The trainer prototype now includes the sequence; the canonical page still needs the editorial retrofit.
