...
* [ ] Build and review the authorized 2026-08-10 cross-instrument provisional composition set. Each draft must remain unpublished and visibly incomplete, reproduce the full canonical SOP on Side 1, use only standardized descriptive frames for unavailable images, and preserve unresolved curriculum or technical choices as review gaps. This pass inventories composition and evidence; it does not make any blocked guide technically complete or usable at an instrument. **The eleven drafts were built and audited on 2026-08-10; the findings and required corrections are in [`../_staff/quick-guides/audit-2026-08-10.md`](../_staff/quick-guides/audit-2026-08-10.md). Work that document rather than re-auditing the set.**
* [ ] Resolve the audit's publication-stopping layout finding: in the real paginated print path the trained-user notice overlaps the SOP heading on **all eleven** prototypes, obscuring one to two lines wherever the instrument title wraps to two lines (Phenom XL, Phenom Pure, XRD, Instron, optical, Raman). This is locked header geometry, so it needs a staff decision. Do not fix it by shortening a canonical instrument title, shrinking type, or dropping an SOP step.
...
* [ ] Rebalance the three pilots toward at-instrument action. Reduce extended interpretation, retain only the immediate quality gate, and add selected hardware or software control-finding visuals where the paper copy prevents browser/software toggling. The unpublished [FTIR SOP/visual-map prototype](../quick-guides/ftir-sop-map-prototype.md) is the approved drafting model as of 2026-08-07; use the [locked production procedure](../_staff/quick-guides/README.md), not ad hoc propagation. Note that the [2026-08-10 audit](../_staff/quick-guides/audit-2026-08-10.md) found the FTIR file's composition transfers but its **image and control-key treatment should not be copied forward** until recaptured and reconciled.
* [ ] Print-test every guide on US Letter at 100% in duplex, grayscale, and color. Check legibility, clipping, page count, long-edge flip, and whether it can be followed while the control software remains fullscreen. The compact FTIR prototype renders as two explicit pages with no clipping, **but the 2026-08-10 audit found its trained-user notice overlaps the SOP heading in the print path, and its three source images are 750x468 frames whose labels are illegible at print scale** — see [`../_staff/quick-guides/audit-2026-08-10.md`](../_staff/quick-guides/audit-2026-08-10.md). It still needs the physical proof and lab-assistant content review. Grayscale reproduction of the stop/ready panels remains unverified for all eleven. Optical and XRD keep their older layout until separately authorized for retrofit.
