...
* [ ] Print-test every guide on US Letter at 100% in duplex, grayscale, and color. Check legibility, clipping, page count, long-edge flip, and whether it can be followed while the control software remains fullscreen. **FTIR was relaid out 2026-08-05** as a continuous two-column flow and verified at 2 pages with no clipping via headless-Chrome render; a physical proof is still wanted for legibility, grayscale QR scanning, and long-edge flip. Optical and XRD keep the older fixed-height two-sheet layout; both were checked on 2026-08-05 and currently print complete, but that layout clips silently if content ever grows past one page per side, so re-check after any content addition.
